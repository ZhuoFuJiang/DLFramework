...
import dozero.functions_conv as FC
from dozero.utils import pair
...
class Conv2d(Layer):
    ...
    def forward(self, x):
        if self.W.data is None:
            ...
            self._init_W(xp)
        y = FC.conv2d(x, self.W, self.b, self.stride, self.pad)
        return y
...
class Deconv2d(Layer):
    ...
    def forward(self, x):
        ...
        y = FC.deconv2d(x, self.W, self.b, self.stride, self.pad)
        return y
...
class RNN(Layer):
    ...
    def forward(self, x):
        if self.h is None:
            ...
            h_new = F.tanh(self.x2h(x) + self.h2h(x))
        self.h = h_new
        return h_new
