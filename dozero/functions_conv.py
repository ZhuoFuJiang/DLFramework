from dozero.functions import Function, linear, broadcast_to
from dozero.utils import pair, get_conv_outsize, get_deconv_outsize
from dozero.core import as_array, as_variable
from dozero import cuda
import numpy as np
...
class Conv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        # W的形状为: OC C KH KW
        KH, KW = W.shape[2:]
        # col的形状为: N, C, KH, KW, OH, OW
        col = im2col_array(x, (KH, KW), self.stride, self.pad, to_matrix=False)

        # tensordot是矩阵乘法，输出的形状为未被选中的轴，值为选取的子数组做点乘运算np.sum(a * b)
        # 下面y的输出形状为: N OH OW OC
...
def im2col_array(img, kernel_size, stride, pad, to_matrix=True):
    ...
    if xp != np:
        ...
        # 这里和深度学习入门的卷积一张一样，但是pad多了SH - 1
        img = np.pad(img, ((0, 0), (0, 0), (PH, PH + SH - 1),
                           (PW, PW + SW - 1)), mode='constant', constant_values=(0, ))
        col = np.zeros((N, C, KH, KW, OH, OW), dtype=img.dtype)

        for j in range(KH):
            # 输出总共OH个值，每个值是卷积核通过SH移动
            j_lim = j + SH * OH
            for i in range(KW):
                i_lim = i + SW * OW
...
def col2im_array(col, img_shape, kernel_size, stride, pad, to_matrix=None):
    N, C, H, W = img_shape
    KH, KW = pair(kernel_size)
    SH, SW = pair(stride)
    PH, PW = pair(pad)
    ...
    if xp != np:
        ...
        # 这里一个x y的权重会流向x:x_max:stride和y:y_max:stride
        # 由于y最大为OH，因此对应的最大图像坐标OH * stride + KH - 1, 则为KH-1 + stride+H+2*pad-KH=H+2*pad+stride-1
